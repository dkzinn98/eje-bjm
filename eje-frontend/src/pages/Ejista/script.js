// scriptEjista.js - Lógica da página Ejista
...
export const EJISTA_DATA = {
  ...
  messages: {
    ...
    error: 'Erro ao realizar inscrição. Por favor, verifique os dados e tente novamente.',
    ...
    invalidAge: 'Idade deve estar entre 14 e 25 anos.',
...
export const useEjistaForm = () => {
  ...
  const validateField = useCallback((fieldName, value) => {
    const rules = EJISTA_DATA.validationRules[fieldName];
    if (!rules) return '';

    if (fieldName === 'email' && !rules.pattern.test(value)) {
      return EJISTA_DATA.messages.invalidEmail;
    }

    if (fieldName === 'whatsapp' && !rules.pattern.test(value)) {
      return EJISTA_DATA.messages.invalidWhatsApp;
    }

    if (fieldName === 'instagram' && !rules.pattern.test(value)) {
      return EJISTA_DATA.messages.invalidInstagram;
    }

    if (fieldName === 'idade') {
      const age = Number.parseInt(value);
      if (age < rules.min || age > rules.max) {
        return EJISTA_DATA.messages.invalidAge;
      }
    }
    ...
    if (fieldName === 'nome' || fieldName === 'apresentacao') {
      if (value.length < rules.minLength) {
        return fieldName === 'apresentacao' 
          ? EJISTA_DATA.messages.shortPresentation
  ...
  // Validação completa do formulário
    ...
    if (!inputRefs.terms.current?.checked) {
      newErrors.terms = EJISTA_DATA.messages.termsRequired;
      isValid = false;
    }
...
// HOOK PARA API
// ========================
export const useEjistaAPI = () => {
  const [uploadProgress, setUploadProgress] = useState(0);

  const submitEjista = useCallback(async (formData, inputRefs) => {
    try {
      ...
      if (inputRefs.fotoDePerfil.current.files[0]) {
        ...
        // Adicionar dados do ejista
        // biome-ignore lint/complexity/noForEach: <explanation>
                        Object.keys(ejistaData).forEach(key => {
          if (key !== 'profileImage') {
            formDataWithFile.append(key, ejistaData[key]);
          }
        });
        ...
        const response = await api.post('/ejista', formDataWithFile, {
          headers: {
      ...
      // biome-ignore lint/style/noUselessElse: <explanation>
        ...
        const response = await api.post('/ejista', ejistaData);
        return response.data;
      }
    } catch (error) {
...
// ========================
// HOOK PRINCIPAL DO EJISTA
// ========================
export const useEjista = () => {
  const form = useEjistaForm();
  const api = useEjistaAPI();
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      await api.submitEjista(null, form.inputRefs);
      ...
      alert(EJISTA_DATA.messages.success);

    } catch (error) {
      console.error('Erro ao cadastrar:', error);
      alert(EJISTA_DATA.messages.error);
    } finally {
  ...
  return {
    ...
    // Dados
    formFields: EJISTA_DATA.formFields,
    messages: EJISTA_DATA.messages
  };
};
...
export const EjistaUtils = {
  ...
  log: (message, type = 'info', data = null) => {
    ...
    console.log(`${emoji[type]} [Ejista] ${message}`, data ? data : '');
  }
};
